...
export function serviceClient(): SupabaseClient {
  ...
  if (!url || !key) {
    throw createError({
      statusCode: 500,
      statusMessage: 'Supabase URL/service_role 키가 설정되지 않았습니다 (.env 확인).',
    })
  }
...
export async function requireAdmin(event: H3Event): Promise<string> {
  ...
  if (error || !data.user) {
    throw createError({ statusCode: 401, statusMessage: '유효하지 않은 세션입니다.' })
  }

  const email = (data.user.email || '').toLowerCase()
  const allow = adminEmails()
  if (allow.length === 0 || !allow.includes(email)) {
    throw createError({ statusCode: 403, statusMessage: '관리자 권한이 없습니다.' })
  }
  return email
}
